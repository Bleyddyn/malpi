""" A Fastai/Pytorch autoencoder for use with a DonkeyCar robot.

    Based on: https://github.com/araffin/aae-train-donkeycar/blob/master/ae/autoencoder.py
"""
...
class Autoencoder(nn.Module):
    ...
    def __init__(self, z_size, input_dimension, learning_rate=0.0001, normalization_mode="rl"):
    ...
    def _build(self, input_shape):
        ...
        self.encoder = nn.Sequential(
            #PrintSize(),
            nn.Conv2d(input_shape[0], 32, kernel_size=3, stride=2, padding=enc_pad),
            nn.ReLU(),
            nn.Conv2d(32, 64, kernel_size=3, stride=2, padding=enc_pad),
            nn.ReLU(),
            nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=enc_pad),
            nn.ReLU(),
            nn.Conv2d(64, 128, kernel_size=3, stride=2, padding=enc_pad),
            nn.ReLU(),
            nn.Conv2d(128, 128, kernel_size=3, stride=2, padding=enc_pad),
            nn.ReLU(),
        )
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.ConvTranspose2d(32, input_shape[0], kernel_size=2, stride=2, output_padding=0),
            nn.Sigmoid(),
            #PrintSize(),
        )
