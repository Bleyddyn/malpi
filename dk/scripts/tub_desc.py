...
Usage:
    tub_desc.py [--file=<file> ...] [--stats] [<dir>...]
...
def describe_tub( tub_dir, stats=False ):
    ...
    if stats:
        ...
        st = ""
    print( "{}\t{}\t{}\t{}\t{}\t{}\t{}{}".format( os.path.basename(tub_dir), count, loc, task, driver, throttle, steering, st ) )
    return count

def check_path( apath, counter=None, stats=False ):
    if os.path.isdir(apath):
        meta_path = os.path.join( apath, "meta.json" )
        if os.path.exists(meta_path):
            cnt = describe_tub(apath, stats)
...
# This will look in the directory containing this script, not necessarily the current dir
    ...
    if do_stats:
        ...
        stat_str = ""
    print( "\nTub\t# Samples\tLocation\tTask\tDriver\tThrottle\tSteering{}".format( stat_str ) )
    for adir in dirs:
        if adir not in done and os.path.isdir(adir):
            if not check_path(adir, counter=counts, stats=do_stats):
                for afile in os.listdir(adir):
                    fpath = os.path.join(adir, afile)
                    if fpath not in done:
                        check_path( fpath, counter=counts, stats=do_stats )
                        done.append(fpath)
            else:
                done.append(adir)
