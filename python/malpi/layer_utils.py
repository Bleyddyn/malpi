...
def conv_relu_forward(x, w, b, conv_param, mode='train'):
  """
...
def conv_sbn_relu_forward(x, w, b, conv_param, sbn_param):
  """
  A convenience layer that performs a convolution followed by a spatial batchnorm followed by a ReLU.

  Inputs:
  - x: Input to the convolutional layer
  - w, b, conv_param: Weights and parameters for the convolutional layer
  - sbm_param: parameters for the spatial batchnorm layer
  ...
  """
  a, conv_cache = conv_forward_fast(x, w, b, conv_param)
  out, sbn_cache = spatial_batchnorm_forward(a, sbn_param)
...
def conv_sbn_relu_backward(dout, cache):
  ...
  da = relu_backward(dout, relu_cache)
  da = spatial_batchnorm_backward(da, sbn_cache)
  dx, dw, db = conv_backward_fast(da, conv_cache)
  return dx, dw, db
